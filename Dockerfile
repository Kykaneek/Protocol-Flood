...
WORKDIR /src
COPY ["ServerTCP.csproj", "."]
RUN dotnet restore "./ServerTCP.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./ServerTCP.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./ServerTCP.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "ServerTCP.dll"]